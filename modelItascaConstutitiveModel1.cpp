#include "modelItascaConstutitiveModel1.h"
#include "version.txt"
...
extern "C" EXPORT_TAG void *createInstance() {
    models::ModelMason *m = new models::ModelMason();
    return (void *)m;
}
// excerpt-export-end
...
// excerpt-con-start
    ModelMason::ModelMason(unsigned short option) : ConstitutiveModel(option) {
    }
// excerpt-con-end

    string ModelMason::getName() const { 
#ifdef MODELDEBUG
        return "mason-debug"; 
#else
        return "mason";
    ...
    string ModelMason::getFullName() const { 
#ifdef MODELDEBUG
        return "Mason Debug";
#else
        return "Mason";
    ...
    uint32 ModelMason::getMinorVersion() const {
        return UPDATE_VERSION;
    }

    string ModelMason::getProperties() const {
        return "bulk,shear,young,poisson,cohesion,friction,dilation,tension,flag-brittle";
    }

    string ModelMason::getStates() const {
        return "shear-n,tension-n,shear-p,tension-p";
    }

    base::Property ModelMason::getProperty(uint32 index) const {
        switch (index) {
        case 1:
            return bulk_;
        case 2:
    ...
    void ModelMason::setProperty(uint32 index, const base::Property& p, uint32 restoreVersion) {
    ...
    bool ModelMason::isPropertyAdvanced(uint32 i) const {
        if (i==9) 
            return true;
        return false;
    }

    void ModelMason::copy(const ConstitutiveModel *m) {
        const ModelMason *mm = dynamic_cast<const ModelMason *>(m);
...
// excerpt-run-start
    void ModelMason::initialize(uint32 d,State *s) {
    ...
    void ModelMason::run(uint32 d,State *s) {
...
//excerpt-run-end	
    ...
    double ModelMason::moduliReduction(const double &factor) {
    ...
    void ModelMason::apexCorrection(const double &fric,State *s,DVect3 *prin,uint32 *iPlasticity,bool bBrittle) {
        if (fric > 0.0) {
    ...
    void ModelMason::tensionCorrection(State *s,DVect3 *prin,uint32 *iPlasticity,const double &ftz,bool bBrittle) {
        s->state_ |= tension_now;
        if (bBrittle) tension_ = 0.0;
    ...
    void ModelMason::shearCorrection(State *s,DVect3 *prin,uint32 *iPlasticity,const double &fs) {
...
// excerpt-state2-end		
    ...
    double ModelMason::getStrengthStressRatio(const SymTensor &st) const {
    ...
    void ModelMason::scaleProperties(const double &scale,const std::vector<uint32> &props) {
